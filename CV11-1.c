...
/*
// V záložce properties>debugging>command argumnets // lze zadat parametry příkazového řádku

/// <summary>
/// main
/// </summary>
///// <param name="argc">- udává počet parametrů</param>
/// <param name="argv">- pointer na </param>
/// <returns></returns>
int main(int argc, char *argv[])
{
	if (argc < 3) // kontrola počtu
	{
		fprintf(stderr, "Chyba - nebylo zadano jmeno vstupniho a vystupniho souboru");
		return 1;
	}

	// Tisknu jednotlivé parametry příkazového řádku
	printf("argc: %s\n", argc]);
	printf("arg[0]: %s\n", argv[0]); // jméno spouštěného programu
	printf("arg[1]: %s\n", argv[1]); // jméno vstupního souboru
	printf("arg[]: %s\n", argv[2]); // jméno výstupního souboru


	FILE* fw = fopen(argv[2], "w"); // otevřu soubor z pole argumentů argv

	if (fw == NULL) // oveření zda se otevřelo 
	{	
	...
	{
		// fprintf(
		if (fprintf(fw, "%d\n", i) < 0) // pokud vrátí funkce frpintf zápornou hodnotu víme, že došlo ke chybě
		{
			fprintf(stderr, "Chyba pri otevirani vstupniho souboru!\n"); // stderr chybový stream
			fclose(fw);
			return 8;
...
int main(int argc, char* argv[]) // argc a argv je prostě dané!
{
	...
	if (argc < 3) // kontrola počtu otevřených souborů (streamů)
	{
		fprintf(stderr, "Chyba - nebylo zadano jmeno vstupniho a vystupniho souboru");
		return 1;
	}
